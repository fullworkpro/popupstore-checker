...
def check_cookie_login(cookie: str, timeout=20) -> bool:
    """先花 1 秒确认 Cookie 是否处于登录态，避免无效 Cookie 白跑一整轮。

    判据：GET https://m.weibo.cn/api/config → data.login。
    未登录时所有时间线/搜索接口都会返回 ok=-100，跑完才知道就太晚了。
    """
    import urllib.request
    headers = {"Cookie": cookie, "User-Agent": MOBILE_UA, "Referer": "https://m.weibo.cn/",
               "X-Requested-With": "XMLHttpRequest", "Accept": "application/json"}
    try:
        req = urllib.request.Request("https://m.weibo.cn/api/config", headers=headers)
        with urllib.request.urlopen(req, timeout=timeout) as r:
            d = json.loads(r.read().decode())
        return bool((d.get("data") or {}).get("login"))
    except Exception as e:
        log(f"Cookie 自检请求失败：{type(e).__name__} {e}")
        return False
...
def main():
    ...
    if args.from_json:
        ...
        if cookie:
            if check_cookie_login(cookie):
                log("Cookie 自检：已登录 ✅")
            else:
                raise SystemExit(
                    "Cookie 自检：微博认为当前不是登录态（api/config → login=false）。\n"
                    "  · 请确认是在 https://m.weibo.cn/ （手机端站点）登录后复制的 Cookie，"
                    "PC 端 weibo.com 的 Cookie 移动端接口不认；\n"
                    "  · 复制位置：F12 → Network → 任选一个 m.weibo.cn 的请求 → "
                    "Request Headers → Cookie 整串；\n"
                    "  · 复制后覆盖写入 " + str(COOKIE_FILE) + " 再重跑本脚本。"
                )
